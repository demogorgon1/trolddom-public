			...
			struct Requirement
			{
					...
					const SourceNode*		aSource)
				{
					...
					aSource->ForEachChild([&](
						const SourceNode* aChild)
					{
						if(aChild->m_name == "id" && (m_type == TYPE_MUST_HAVE_AURA || m_type == TYPE_MUST_NOT_HAVE_AURA))
							m_id = aChild->m_sourceContext->m_persistentIdTable->GetId(DataType::ID_AURA, aChild->GetIdentifier());
						else
							TP_VERIFY(false, aChild->m_debugInfo, "'%s' is not a valid item.", aChild->m_name.c_str());
					});
				}
